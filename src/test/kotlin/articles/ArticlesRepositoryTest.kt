...
class ArticlesRepositoryTest {
    ...
    fun setup() = runBlocking {
        ...
        // Create test users
        userEntity =
            usersRepository.findUserEntityByEmail("user@example.com")
                ?: usersRepository.createUser("user", "user@example.com", "password", "salt").let {
                    usersRepository.findUserEntityByEmail("user@example.com")!!
                }
    ...
    fun testCreateArticle() = runBlocking {
        ...
        assertEquals(
            "test-article",
            articleEntity.slug,
        ) // Assuming toSlug() converts "Test Article" to "test-article"
    ...
    @Test
    fun testGetArticleBySlug_ExistingArticle() = runBlocking {
        // Given
        val createArticleDto =
            CreateArticleDto(
                title = "Test Article",
    ...
    @Test
    fun testGetArticleBySlug_NonExistingArticle() = runBlocking {
        // Given
    ...
    fun testUpdateArticle() = runBlocking {
        ...
        assertEquals(
            "updated-title",
            updatedArticle.slug,
        ) // Assuming toSlug() converts "Updated Title" to "updated-title"
    ...
    @Test
    fun testUpdateArticle_PartialUpdate() = runBlocking {
        // Given
        val createArticleDto =
            CreateArticleDto(
                title = "Original Title",
    ...
    @Test
    fun testGetAllArticles_NoFilter() = runBlocking {
        // Given
    ...
    @Test
    fun testGetAllArticles_WithAuthorFilter() = runBlocking {
        // Given
        ...
        val user1Articles =
            articlesRepository.getAllArticles(author = userEntity, limit = 10, offset = 0)

        // Then
        assertEquals(3, user1Articles.size)
        user1Articles.forEach { article -> assertEquals(userEntity.id, article.authorId) }

        // When - Get articles by second user
        val user2Articles =
            articlesRepository.getAllArticles(author = anotherUserEntity, limit = 10, offset = 0)

        // Then
        assertEquals(2, user2Articles.size)
        user2Articles.forEach { article -> assertEquals(anotherUserEntity.id, article.authorId) }
    }

    @Test
    fun testGetAllArticles_WithPagination() = runBlocking {
        // Given
    ...
    @Test
    fun testGetAllArticles_OrderByCreatedAtDesc() = runBlocking {
        // Given
        val totalArticles = 5
    ...
    @Test
    fun testGetAllArticles_WithAuthorsList() = runBlocking {
        // Given
        // Create articles by first user
        repeat(3) { index ->
            val createArticleDto =
        ...
        val thirdUserEntity =
            usersRepository.createUser("third", "third@example.com", "password", "salt").let {
                usersRepository.findUserEntityByEmail("third@example.com")!!
            }
        ...
        val retrievedArticles =
            articlesRepository.getAllArticles(authors = authorsList, limit = 10, offset = 0)
        ...
        val paginatedArticles =
            articlesRepository.getAllArticles(authors = authorsList, limit = 2, offset = 0)
        assertEquals(2, paginatedArticles.size)

        val secondPageArticles =
            articlesRepository.getAllArticles(authors = authorsList, limit = 2, offset = 2)
