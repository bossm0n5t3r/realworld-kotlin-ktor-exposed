...
class ArticlesServiceIntegrationTest {
    ...
    private val favoriteArticlesRepository: FavoriteArticlesRepository =
        FavoriteArticlesRepository(databaseManager)
    ...
    fun setup() = runBlocking {
        ...
        // Get user IDs
        user1Id =
            usersRepository.findUserEntityByEmail("user1@example.com")?.id?.value?.toString()
                ?: error("User not found")
        user2Id =
            usersRepository.findUserEntityByEmail("user2@example.com")?.id?.value?.toString()
                ?: error("User not found")
    }
    ...
    fun testGetAllArticlesWithAuthorFilter() = runBlocking {
        ...
        result.articles.forEach { article -> assertEquals(user1Username, article.author.username) }
    }
    ...
    fun testGetAllArticlesWithTagFilter() = runBlocking {
        ...
        result.articles.forEach { article -> assertTrue(article.tagList.contains("tag1")) }
    }
    ...
    @Test
    fun testGetAllArticlesWithFollowingRelationship() = runBlocking {
        // Given
        val user1Entity = usersRepository.getUserEntityById(user1Id)
        val user2Entity = usersRepository.getUserEntityById(user2Id)
    ...
    @Test
    fun testGetAllArticlesWithLimitAndOffset() = runBlocking {
        // Given
        val user1Entity = usersRepository.getUserEntityById(user1Id)
    ...
    fun testUpdateArticleNotAuthor() = runBlocking {
        ...
        val updateArticleDto = UpdateArticleDto(title = "Updated By User2")
    ...
    fun testGetCommentsForArticle() = runBlocking {
        ...
        assertTrue(
            result.comments.any { it.body == comment1Body && it.author.username == user1Username }
        )
        assertTrue(
            result.comments.any { it.body == comment2Body && it.author.username == user2Username }
        )
        ...
        val user2CommentAfterFollow =
            resultAfterFollow.comments.find { it.author.username == user2Username }
