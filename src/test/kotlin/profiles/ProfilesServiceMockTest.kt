...
class ProfilesServiceMockTest {
    ...
    private val profilesService: ProfilesService =
        ProfilesService(usersRepository, followingsRepository)

    @Test
    fun testGetProfileWhenNotFollowing() = runBlocking {
        // Given
    ...
    @Test
    fun testGetProfileWhenFollowing() = runBlocking {
        // Given
        val username = "testuser"
        val currentUserId = UUID.randomUUID().toString()
        val userId = UUID.randomUUID()
    ...
    @Test
    fun testGetProfileWithoutCurrentUser() = runBlocking {
        // Given
        val username = "testuser"
        val userId = UUID.randomUUID()
        val userEntity = mockk<UserEntity>()
    ...
    @Test
    fun testGetProfileWithNonExistentUsername() = runBlocking {
        // Given
    ...
    fun testFollow() = runBlocking {
        ...
        coEvery { usersRepository.findUserEntityByUsername(username) } returns userEntity
        coEvery { usersRepository.getUserEntityById(currentUserId) } returns currentUserEntity
        coEvery { followingsRepository.addFollowing(userId.toString(), currentUserId) } returns
            mockk()
    ...
    @Test
    fun testFollowNonExistentUser() = runBlocking {
        // Given
        val username = "nonexistentuser"
        val currentUserId = UUID.randomUUID().toString()
        ...
        // When/Then
        assertFailsWith<IllegalStateException> { profilesService.follow(username, currentUserId) }
    ...
    fun testUnfollow() = runBlocking {
        ...
        coEvery { usersRepository.findUserEntityByUsername(username) } returns userEntity
        coEvery { usersRepository.getUserEntityById(currentUserId) } returns currentUserEntity
        coEvery { followingsRepository.deleteFollowing(userId.toString(), currentUserId) } returns
            Unit
    ...
    fun testUnfollowNonExistentUser() = runBlocking {
        ...
        // When/Then
        assertFailsWith<IllegalStateException> { profilesService.unfollow(username, currentUserId) }
